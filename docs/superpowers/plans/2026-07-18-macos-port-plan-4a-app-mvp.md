...
**Task 12 amendment (recorded 2026-07-18, reviewer-adjudicated):** `engine/fonts.cpp` `CUnitPanelPage::DestroyFonts` gained one statement — `ccContext().metricsDC()->SelectObject((CFont*)NULL);` before the font delete loop — ratified as an **R13-clarification instance** (minimal, output-preserving runtime-safety fix). The original 1998 code was correct on Windows (per-use `CClientDC`, always freshly-selected stock font); the port's single long-lived cached metrics DC retains a stale `CFont*` across strip destroy/create, and `SetFonts`' select/restore round-trips it back in before the delete loop frees it — dangling-pointer read on the next measurement (found LIVE via the app's `setViewport` reflow; no selftest catches it because every selftest re-registers the metrics canvas, which resets the DC). The NULL is transient and unread (next `SetFonts` overwrites before any measurement); `currentFontSpec()` treats null as the default spec. Placement in `DestroyFonts` is correct — it is the only place these fonts are freed, and its sole caller is `cc_strip_destroy` (a review-brief claim that `UpdateTitleFonts` also calls it was verified FALSE).
