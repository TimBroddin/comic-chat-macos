...
**R13 clarification (amendment, ratified 2026-07-18 during Task 3 review).** R13 was written for *compile-time* pre-standard MSVC-isms clang rejects. It also covers, by the Plan 2 Task 8 precedent (`panel.cpp` LP64 `(DWORD)(uintptr_t)` pointer-truncation casts) and now by this ratification, **minimal, output-preserving RUNTIME safety fixes** — an OOB read, a pointer-width hazard on LP64 — where the original compiles but misbehaves on this 64-bit target. Same discipline: minimal, behavior-preserving for every in-bounds/valid case, each instance individually listed with before/after and a note that only the crash-vs-not (or truncation) behavior changes. Task 3's `FindInKeyString` OOB-guard fix is the first instance ratified under this clarification.

**Preserved-forever quirk (interop fidelity, recorded 2026-07-18 during Task 3 review).** `ChangeKeyString`'s deletion path (`protsupp.cpp` original :5142–5157) computes the deleted result into a local but returns TRUE **without writing it back** — key deletion is a genuine no-op in the 1998 code; write-back happens only on the insertion path (:5182). This is lifted VERBATIM (buggy) and must **never be "fixed"**: a working deletion would diverge from real Comic Chat clients on the wire (same class as the preserved textpose `lptr` bug and the restored CGCanvas mirror behavior). Any later task wiring `ChatSetClientData`/`ChangeProperty` (backdrop `bk=` key management) inherits the no-op deletion — do not correct it.
...
- [ ] **Step 5: Implement `ProtocolSession`** — owns `cc_session` (config callbacks bridge to Swift closures via an `Unmanaged<AnyObject>` `user_data`); `send` writes to the `NWConnection`; `NWConnection`'s receive handler calls `cc_session_feed_bytes`; `set_timer`/`cancel_timer` schedule a `DispatchSourceTimer` that calls `cc_session_fire_timer`; `on_event` maps the C union → `ProtocolEvent`, updates the state model, and yields into the `AsyncStream`; `own_nick`/`resolve_user` answer from the Swift state model. The state model tracks exactly the `state-and-codec.md` §1.4 set (own nick with the echo-only update rule, per-room membership with soft-departs, user metadata, room props, whisper targets, connection status).
  ...
  **IRCX DATA→PRIVMSG annotation re-pairing (assigned here 2026-07-18 during Task 6 review — Important).** The engine's payload classifier is deliberately **stateless** (Task 6): it holds no persistent per-user `m_udi`, so the original's two-message IRCX pairing (a `DATA <target> CCUDI1 :#G…` blob stashed on the sender's `CUserInfo.m_bbValidUDI`, then consumed by the *next* plain `PRIVMSG` from that sender — `state-and-codec.md` §3.3) does **not** happen inside the engine. On the IRCX transport the engine therefore emits `CC_EV_DATA{nick, annotations}` and then, for the paired plain PRIVMSG, `CC_EV_TEXT{nick, has_annotations=0}`. **`ProtocolSession` re-pairs by nick:** keep a per-nick "pending UDI" slot; on `CC_EV_DATA` store the annotations under that nick; on the next `CC_EV_TEXT` (or `.whisper`/`.action`) from the same nick with `has_annotations==0`, attach the pending annotations and clear the slot (a DATA blob is single-use — `m_bbValidUDI=0` after consume). Plain-IRC inline `(#…)` annotations are self-contained per-message (the engine decodes them and sets `has_annotations=1`), so they need no re-pairing — only the IRCX out-of-band path does. Test this explicitly: feed a scripted `DATA … CCUDI1 :#G…` line followed by a plain `PRIVMSG` from the same nick and assert the yielded `.text` carries the pose from the DATA blob.
