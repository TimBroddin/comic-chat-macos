//#include "stdafx.h"
// R8 (Plan 2 Task 6, header-only lift): panel.h carried no active includes
// in the original (its only #include, "stdafx.h", was already commented out;
// it relied on the includer -- balloon.cpp/fonts.cpp -- pulling in every type
// it names first). To keep it self-consistent when included, forward-declare
// the three UI/doc/print types it uses pointer-opaquely (CChatDoc, CPageView,
// CPrintInfo). The originals lived in chatdoc.h/pageview.h/<afxwin.h>, all R8
// UI headers deleted from the lifting TUs; panel.h never dereferences any of
// them (all uses are `Type*` in signatures + one `CChatDoc*` data member set
// by pointer in the ctor), so an opaque forward declaration is exact.
class CChatDoc;    // R8: was chatdoc.h
class CPageView;   // R8: was pageview.h
class CPrintInfo;  // R8: was MFC <afxwin.h> (printing framework)
// DECLARE_DYNAMIC (used by CDamage below) is defined in mfc_compat.h (R9).
