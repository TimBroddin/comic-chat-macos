...
#include "backdrop.h"
#include "vector2d.h"
// R8: "userinfo.h", "ui.h", "chatprot.h", "binddoc.h", "chatdoc.h", "histent.h" dropped (UI/doc headers)

// R5: <io.h> dropped (backdrop directory scan; wrapped below under CC_NO_DIRSCAN)
...
// build file search strings
	...
	for (iType = 0; iType < NUMBACKDROPTYPES; iType++) {
		strPattern = (LPCSTR) ccContext().backdropDir;  // R2 (was theApp.GetBackDropDir())
...
#ifndef CC_NO_UI  // R11: session/doc layer (theApp.m_lastBackDrop, GetChatDoc, download state), not load/parse path
...
void SetBackDrop(const char *backname, const char * backURL) {
	...
	if (backIndex == -1) {
		...
		if (!OurMbsChr (backname, '.'))
		{
			...
			{
				filename.Format("%s/%s%s", (LPCSTR) ccContext().backdropDir, backname, BACKDROPTYPE_FILEEXT(iType));  // R2 + R3 (was theApp.GetBackDropDir(), "%s\\%s%s")
		...
		{
			filename.Format("%s/%s", (LPCSTR) ccContext().backdropDir, backname);  // R2 + R3 (was theApp.GetBackDropDir(), "%s\\%s")
...
#ifndef CC_NO_UI  // R11: session/doc layer (theApp.m_lastBackDrop), not load/parse path
...
CBackDropArt *BackDropArtFromBackID(UINT backID, BOOL toScreen) {
	...
	buff.Format("%s/%s", (LPCSTR) ccContext().backdropDir, rec->filename);  // R2 + R3 (was theApp.GetBackDropDir(), "%s\\%s")
