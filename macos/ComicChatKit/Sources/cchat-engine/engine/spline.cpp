...
void CCardinal::SetMatrix(double tension) {
	...
	matrix = (MATRIX *) malloc (sizeof(MATRIX));  // R13: MSVC allows a bare type name after sizeof; clang requires parens
...
int dash_sample(DPOINT *p, void *arg) {
	...
	// R13: MSVC allows binding the dpoint_to_point(*p) temporary directly to
	// DashSeg's non-const POINT& parameter; clang requires a named local.
	POINT thisPoint = dpoint_to_point(*p);
	DashSeg(thisPoint, *((DASHINFO *)arg));
