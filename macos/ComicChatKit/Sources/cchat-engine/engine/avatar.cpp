...
#include "dib.h"
// R8: "userinfo.h", "ui.h", "chatprot.h", "memblst.h", "binddoc.h", "chatdoc.h" dropped (UI/doc headers)
#include "avatar.h"
#include "vector2d.h"
#include "math.h"
// R5: <io.h> dropped (avfile directory scan; wrapped below under CC_NO_DIRSCAN)

#ifndef CC_NO_UI  // R11: body-cam refresh is UI/session layer, not load/parse path
...
CBody *CAvatarComplex::GetBodyFromEmotion(CEmotion &emotion) {
	...
	double intensityOfNearest = 2.0;
	int nearestI;
	int i;  // NOTE: hoisted out of the loop below (was `for (int i ...)`); the
	        // original 1998 source relies on pre-standard MSVC `for`-scope
	        // leakage (i still visible after the loop) to reuse `i` in the
	        // second loop a few lines down. Standards-conforming C++ scopes
	        // `i` to the loop, so the second loop's bare `i = 0` wouldn't
	        // resolve. Not a Win32/MFC API — a core-language portability
	        // fix, minimal and behavior-preserving (same iteration order,
	        // same values), not covered by R1-R11; flagged in the report.
...
void CAvatarSimple::SetNeutral() {
	// NOTE: original passed the CEmotion temporary directly to
	// GetBodyFromEmotion(CEmotion&); MSVC historically allowed binding a
	// temporary to a non-const reference (a well-known non-conformance),
	// which standards-conforming C++ rejects. Binding to a named local
	// first is behavior-identical (same value passed by reference), not
	// covered by R1-R11; flagged in the report.
	CEmotion neutralEmotion(0.0, 0.0);
	CBody *body = GetBodyFromEmotion(neutralEmotion); // creates the only body that's known...
//	body->m_requested = FALSE;
	UpdateBody(body);
}
...
#ifndef CC_NO_UI  // R11: my-avatar selection is UI/session layer (GetChatDoc/theApp/protocol), not load/parse path
...
// build file search strings
    ...
    strFiles.RemoveAll();
	strPattern = (LPCSTR) ccContext().avatarDir;  // R2 (was theApp.GetAvatarDir())
	strPattern += "/*.avb";  // R3 (was "\\*.avb")
   ...
   #else
	strFiles.RemoveAll();  // stub: no directory scan available (R5)
   #endif
}
...
// R11 vtable exception: SetSequential overrides a pure virtual (avatar.h:227)
// that lifted code instantiates (CAvatarX::LoadAvatar creates CAvatarSimple/
// CAvatarComplex), so the definitions must stay compiled. Their internals are
// entirely UI/session layer (CUserInfo, GetTickCount) and not required by the
// load/parse path, so they're stubbed to a safe no-op outside CC_NO_UI.
