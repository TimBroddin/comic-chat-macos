...
// R2: was "chat.h" (extern CChatApp theApp / theApp.* accessors) -- the only
// chat.h consumers in this file are the CBodyCam widget (all CC_NO_UI-wrapped);
// the live CBody draw methods need none of it, so no engine_context.h include
// is required here.
#include "bbox.h"
#include "pe.h"
#include "dib.h"
...
// R8: saywnd.h / resource.h / ui.h / binddoc.h / chatdoc.h / protsupp.h /
// chatprot.h are all UI/doc/protocol headers used only by the CBodyCam widget
// (CC_NO_UI-wrapped below). Deleted -- the live CBody draw methods reference
// none of their types. userinfo.h IS lifted, but the live methods here don't
// use it either, so it is not included.
#include "vector2d.h"     // ROUND, PI, point/vector helpers (GetBodyBox math)
#include <math.h>
...
// R11: the entire CBodyCam widget half of this file -- the emotion-picker CWnd
// (paint/mouse/tooltip/context-menu handlers, GetBodyCam() singleton,
// theApp.DoOptionsDialog, connection-status reads, retained-DIB machinery) --
// is UI/session layer and is not part of the headless load/parse/draw path.
// It is wrapped whole under CC_NO_UI (defined for this plan): none of it
// compiles headless. Every symbol between here and the matching #endif is one
// R11 exclusion (enumerated individually in p2-task-7-report.md). The ten
// CBody*/CBodyDouble/CBodySingle draw+bbox method definitions that go LIVE for
// this task are AFTER the #endif.
#ifndef CC_NO_UI
...
// ============================================================================
// LIVE: CBody / CBodyDouble / CBodySingle draw + bbox implementations.
// These ten methods are the point of this task. They are declared (byte-
// identical) in avatar.h and were R12(b)-stubbed in cc_link_stubs.cpp for
// Task 4; those ten stubs are deleted now that these real bodies land. They
// stay VERBATIM from bodycam.cpp except the R14(i) mask-ROP-pair collapse in
// the two DrawBody methods (documented inline + in p2-task-7-report.md).

// verbatim from bodycam.cpp:447
...
// from bodycam.cpp:516 -- R14(i) transformed (see the four numbered comments).
// Each original mask-ROP pair
//     mask->Draw(dc, l,t,w,h, MERGEPAINT);   // (NOT mask) OR dest
//     drawing->Draw(dc, l,t,w,h, SRCAND);     // drawing AND dest
// collapses to ONE dc->DrawPoseImage(drawing, mask, l,t,w,h): the drawing plane
...
RECT CBodyDouble::DrawBody(CDC *dc, RECT &clientRect, BOOL drawNimbus) {
	...
	if (flags & TORSOFIRST) {
		// (1) torso mask+drawing pair -> one DrawPoseImage(drawing, mask).
		if (torsoPose->GetDrawing ())
			dc->DrawPoseImage(torsoPose->GetDrawing (),
							((flags & TORSOMASK) && torsoPose->GetMask ()) ? torsoPose->GetMask () : NULL,
							torsoRect.left, torsoRect.top,
							torsoRect.right - torsoRect.left, torsoRect.bottom - torsoRect.top);
	}

	// (2) head mask+drawing pair -> one DrawPoseImage(drawing, mask).
	if (headPose->GetDrawing ())
		dc->DrawPoseImage(headPose->GetDrawing (),
					   ((flags & HEADMASK) && headPose->GetMask ()) ? headPose->GetMask () : NULL,
					   headRect.left, headRect.top,
					   headRect.right - headRect.left, headRect.bottom - headRect.top);

	if (!(flags & TORSOFIRST)) {
		// (3) torso mask+drawing pair -> one DrawPoseImage(drawing, mask).
		if (torsoPose->GetDrawing ())
			dc->DrawPoseImage(torsoPose->GetDrawing (),
							((flags & TORSOMASK) && torsoPose->GetMask ()) ? torsoPose->GetMask () : NULL,
							torsoRect.left, torsoRect.top,
...
// from bodycam.cpp:578 -- R13: the original passed the SRECTToRECT(m_bbox)
// temporary directly to DrawBody(CDC*, RECT&, BOOL); MSVC bound the temporary
// to the non-const reference (a well-known non-conformance), which standards-
// conforming C++ rejects. Binding to a named local first is behavior-identical
// (same value passed by reference), not covered by R1-R11; flagged in the
// report. Same instance as avatar.cpp's two R13 fixes.
void CBodyDouble::Draw(CDC *dc, POINT *ul, RECT *dmgRect) {
	// for now, ignore ul and dmgRect
	RECT bboxRect = SRECTToRECT(m_bbox);
	DrawBody(dc, bboxRect, TRUE);
}

// verbatim from bodycam.cpp:583
...
// from bodycam.cpp:589 -- R14(i) transformed. CBodySingle has no separate mask
// blit (the single drawing plane was pre-masked at load: image &= mask, see
...
// from bodycam.cpp:614 -- R13: same non-const-reference-to-temporary fix as
// CBodyDouble::Draw above (SRECTToRECT(m_bbox) hoisted to a named local).
...
// ============================================================================
// R11: remaining CBodyCam widget half (retained-DIB machinery, focus/context/
// palette handlers, GetBodyCam-dependent free helpers). All UI/session layer;
// wrapped whole under CC_NO_UI. Every symbol here is one R11 exclusion.
#ifndef CC_NO_UI
...
#endif // CC_NO_UI  -- end of the remaining CBodyCam widget half (R11)
