...
void DashArc2(DASHINFO &d, POINT& start, POINT& end, int altitude) {
	...
	while (TRUE) {
		...
		DashSeg(M, d);  // R13: was DashSeg(point_add(dpoint_to_point(C), absCenter), d) -- a POINT temporary bound to DashSeg's non-const POINT& param; M holds the identical value
		if (doExit) break;
		A = C;
