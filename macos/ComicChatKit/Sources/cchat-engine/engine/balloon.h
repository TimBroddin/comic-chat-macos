...
#include "format.h"
#include "engine_context.h"  // R2 (was the "extern CChatApp theApp;" below):
                             // supplies ccContext().session (F1_RTFCOMIC read
                             // in the inline CLabel ctor) + defines.h's
                             // F1_RTFCOMIC constant.

#define FT_LEFT_JUSTIFY	1
#define MAXLINES	10

// R2: "extern CChatApp theApp;" deleted; the one theApp read in this header
// (the inline CLabel ctor's theApp.m_flags1) is R17-rerouted below.
...
public:
	...
	{ 
		m_str = strdup(szText);
		m_fontI = pFontInfo;
		m_prgdwFormatting = (ccContext().session.flags1 & F1_RTFCOMIC) ? CopyFormatting(prgdwFormatting) : CopyLinksFormatting(prgdwFormatting);  // R17 (was theApp.m_flags1)
		m_format = (char) 0;
	}
	virtual ~CLabel() 
	{
