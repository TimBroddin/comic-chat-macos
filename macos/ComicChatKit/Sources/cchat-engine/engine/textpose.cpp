...
#include "engine_context.h"  // R2 (was "chat.h"; supplies ccContext().session +
                              // the ID_RULE_* constants + ccSeedEmotionRuleStrings())
...
#include "avatar.h"
#include "bodycam.h"
// R8: ui.h/userinfo.h/chatprot.h/binddoc.h/chatdoc.h/resource.h deleted (UI/
// doc/protocol/resource headers). textpose.cpp names none of their types --
// GetChatDoc() was only reached for m_bComicView (R17-rerouted to
// ccContext().session.comicView below) and CUserInfo/CChatProt/CBindDoc are
// never referenced here. The ID_RULE_* constants resource.h defined now live
// in engine_context.h (R17 — see ccSeedEmotionRuleStrings()).

// R2: "extern CChatApp theApp;" deleted; this file has no theApp.* uses.
...
void ChatPreSendText(CString &str, int avID) {
	...
	if (!ccContext().session.comicView) return;  // R17 (was !GetChatDoc() || !GetChatDoc()->m_bComicView)
