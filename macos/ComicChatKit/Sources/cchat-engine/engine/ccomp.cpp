// ccomp.cpp — lifted from artifacts/core/ccomp.cpp. Plan 3 Task 4.
// See ccomp.h for the provenance/Edit Rules note.
//
// Edit Rules applied, hunk by hunk:
//   R1  - #include "CDebug.H"/"CComp.H" + SZTHISFILE -> #include "ccomp.h"
//         (+ mfc_compat.h transitively). SZTHISFILE's debug-heap machinery is
//         not lifted (R8-style: debug-only diagnostic aid, not comparison
//         logic).
//   R13 - CDebug.H's two-argument ASSERT(cond, "message") -> mfc_compat.h's
//         single-argument ASSERT(cond); the redundant message literal is
//         dropped, condition unchanged (same rewrite Task 2 applied to
//         ccommon_str.cpp).
...
BOOL bGetUserMatchFromMask(LPTSTR szIdentMask, PPRUSERMATCH pPrUserMatch)
{
	...
	if (pPrUserMatch->szIPAddress)
		...
		{
			ASSERT(pPrUserMatch->szNickname);
            pPrUserMatch->cbNickname = iMask - iLast;
		}
