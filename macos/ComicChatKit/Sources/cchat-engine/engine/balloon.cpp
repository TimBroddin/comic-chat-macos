...
#include "userinfo.h"        // lifted now (Task 6)
// R8: chatprot.h/ircproto.h/ui.h deleted (protocol + UI headers). No type from
// them is named by lifted balloon.cpp code; the two protocol/session symbols
// that ircproto.h/chatprot.h would supply (serverConn, CPanel::OnClickHotLink)
// live only inside R11-wrapped bodies (see SplitHeight/OnLButtonDown below).
#include "vector2d.h"
#include "traj.h"
#include "spline.h"
#include "bbox.h"
...
#include "balloon.h"
#include "backdrop.h"
// R8: script.h deleted (verified dead -- nothing in balloon.cpp uses any
// script.h symbol).
// R8: binddoc.h/chatdoc.h/pageview.h deleted (doc/view UI headers; CChatDoc/
// CPageView appear only via panel.h's opaque forward declarations, not here).
#include "panel.h"           // lifted header-only (Task 6)
#include "format.h"
// R8: ccommon.h deleted -- it only ever supplied string constants (g_chEOS,
// g_chTransparent) that mfc_compat.h now provides verbatim (same treatment as
// lifted format.cpp), plus externs for out-of-scope files balloon.cpp doesn't
// call.
#include <ctype.h>
#include <stdlib.h>
#include <math.h>
...
// R2: these two were declared in chat.h (R2-deleted). linkColor is defined
// unconditionally in lifted format.cpp (the hot-link text color, drawn in
// iDrawFormattedTextLine). FLaunchBrowser is defined in format.cpp but only
// under CC_NO_UI-off (R11: URL-launch feature), so its sole balloon.cpp caller
// (CBalloon::OnLButtonDown) is likewise CC_NO_UI-wrapped -- declare it there.
...
// R11 (Plan 2 Task 6): DrawPoints is a debug/visualization helper that calls
// DrawPoint (panel.cpp/chat.h UI free function, not lifted). It is dead in
// balloon.cpp -- nothing calls it -- and purely a rendering aid, so its whole
// definition is wrapped. Restored verbatim when the UI layer lands.
#ifndef CC_NO_UI
...
int PermuteFilters(CFontInfo& fontI, RANGE lFilters[], RANGE rFilters[], int nLFilters, int nRFilters)
{
	...
	baseY = 0;
	lastX = -LARGEINTEGER;
	for (int i = 0; i < nRFilters; i++) {  // R13: was `for (i = ...)` reusing the first loop's `i` (MSVC /Zc:forScope-)
...
char* CLabel::SplitHeight(int iHeight, CDWordArray **pprgdwRestFormatting, char **pszURLStartInRest)
{
	...
	CDC*			pdc = ccContext().metricsDC();  // R17 (was CClientDC* = GetClientDC(); metricsDC returns CDC*)
...
// R11 (Plan 2 Task 6, URL/hot-link interaction path): handles a mouse click on
// a hot-link label -- hit-tests the click against the formatted text and, on a
// URL hit, calls pPanel->OnClickHotLink (a UI CPanel method). Pure UI/session
// interaction, never on the layout path. Overrides CLabel::OnLButtonDown, so
// the definition stays for vtable completeness; body wrapped whole with an
// empty #else (no click handling headlessly). Restored when the UI layer lands.
...
// R11 (Plan 2 Task 6, URL/hot-link interaction path): mouse-click handler for
// a balloon -- hit-tests the click and launches the browser (FLaunchBrowser,
// itself R11-wrapped in format.cpp) on a URL hit. Pure UI/session interaction,
// never on the layout path. Overrides CLabel::OnLButtonDown; definition stays
// for vtable completeness, body wrapped whole with an empty #else. Restored
// when the UI layer lands. FLaunchBrowser is only declared under CC_NO_UI-off.
	...
	*pprgdwRestFormatting = PullFormattingOffsets(m_prgdwFormatting, szRestStart - m_str);
	...
	if (m_prgdwFormatting)
	{
		...
		if (m_prgdwFormatting)
		{
			...
			{
				// R11 (Plan 2 Task 6, URL/hot-link interaction path): this
				// block hands a URL that spans a balloon break off to the next
				// balloon (a hot-link feature). It reaches serverConn
				// (m_nMaxMsgLength -- the protocol/session layer) and is not
				// part of headless layout geometry; SplitHeight's line-count
				// truncation above/below runs unchanged. Wrapped whole so
				// Plan 3's protocol layer restores it verbatim; the #else
				// leaves *pszURLStartInRest NULL (set at function top) --
				// correct, since no URL is carried across balloons headlessly.
				// (Also elides two MSVC /Zc:forScope- reuses that would need
				// R13 -- iFormatIndex at the second for, i at the ASSERT --
				// both excluded here rather than separately rewritten.)
#ifndef CC_NO_UI
...
CSpline* CBWoodringNormal::CreateBalloonSpline(CFormatInfo& fInfo)
{
	...
	for (int i = nRFilters-1; i >= 0; i--) {  // R13: was `for (i = ...)` reusing the prior loop's `i` (MSVC /Zc:forScope-)
