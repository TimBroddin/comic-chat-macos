...
// header. This provenance comment is plan-mandated (Task 5 brief), not a
// fidelity violation -- the rest of the file below is byte-identical to the
// source.
#ifndef __FORMAT_H__
#define __FORMAT_H__
...
// String + Formatting couple
typedef struct tagSTRFMT
{
	CHAR*			szString;			// the string
	CDWordArray*	prgdwFormatting;	// and its formatting
} STRFMT, *PSTRFMT;
...
extern char*		SzReplaceFormattedString(const char *szCLReplaceWhat, const char *szCLReplaceBy, const char *szCLReplaceIn, CDWordArray *prgdwReplaceByFormatting, CDWordArray *prgdwReplaceInFormatting, UINT uFlags);
...
extern void			FreeAndNullFormatting(CDWordArray **pprgdwFormatting);
